...
| ~~**2.21.0 — the CGA null basis**~~ | ✅ **SHIPPED. A conformal point is now EXACTLY null at every magnitude** — 0 of 6800 full-mantissa points over 2^-40..2^40 non-null, against 24.8% before — and distance recovery `P1.P2 = -d^2/2` went from a **median 46.4x wrong at 2^-30 and 48.7x at 2^+30 to ~1.9e-15, FLAT**: sixteen orders at the tails, and scale-free rather than a U centred on |x|~1. `cga_norm(cga_translator(t,0,0))` is exactly 1 for **all 1001 binades** (526 failed before; 2.17.0 recorded failure from t = 2^28). ⭐ **And it is 2.1-2.2x FASTER** — a table lookup replaces the inline bit-manipulation product. ⭐ The repair was the BASIS, not the arithmetic: 2.20.0 proved no arithmetic fix existed, because at x = 2^-30 the correctly-rounded `ep` IS -1/2. ⛔ Two things had to move with it: `_cga_scalar_of_geo` sums ALL pairs (diagonal-only left points non-null 6800 of 6800 — the `-q` lives in the skipped pair), and 2.18.0's Neumaier compensation had to be REMOVED, because it was a mitigation for this very symptom in the old basis and now prevents the exact cancellation. ⛔ Building it found a **cycc 6.6.2 wrong-code bug** (nested `continue` exits the outer loop), filed upstream with a self-proving repro. ⭐ The 2.20.0 acceptance pins INVERTED as designed. Suites 3983 -> 3989. | 2.20.0 |
