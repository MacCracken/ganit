...
Suite **3983** across five harnesses (hisab 496, foundation 413, modules 2065, edge_cases 233,
...
| **2.21.0 — the CGA null basis** | ⭐ **SCOPED, DERIVED, AND THE PRODUCT PLUMBING IS IN (bites 1-3 shipped); the row's size estimate HOLDS.** Bite 2 widened `_cga_geo_blades` to two term slots, bit-identical (`ff49e7071c7f4fc`) with the checksum shown to discriminate AND the new slot shown to be live; measured cost +3.1% dense / +6.4% sparse, the SPARSER operand paying more. Bite 3 pinned the implementation contract: **FNV-1a 0xF4A98C5706D5CF5B** over all 1024 entries in BLADE-INDEX space, plus the defining identities (n0^2 = 0, ninf^2 = 0, n0.ninf = -1). ⛔ **The index space is the hazard and it caught me**: the derivation works in BITMASK space while the implementation is called with blade indices, and my first spot-check read `n0^2 = +1` when it was really `e3^2 = +1` in the other numbering. **A table that is right in the wrong space is the plausible-but-wrong artefact the gate exists to stop.** ⭐ **Bite 4: the table is BUILT and matches** — FNV-1a 0xF4A98C5706D5CF5B, integer arithmetic only, 5 assertions, verified to fail when the metric is flipped; not wired in yet (behaviour checksum still ff49e7071c7f4fc). ⛔ **And building it found a COMPILER BUG**: on cycc 6.6.2 a loop nest with `continue` at two levels miscompiles — a firing inner `continue` exits the OUTER loop, even when the outer one never fires — so the natural sparse-skip form produced an **all-zero table while reporting success**. Filed upstream with a self-proving repro; worked around with `if`-guards. Remaining: wire the table into `_cga_geo_blades`, flip the constructors and `_cga_scalar_of_geo`'s diagonal-only loop, and invert the 2.20.0 acceptance tests. `scripts/derive-cga-null-table.sh` now derives, from first principles and in CI, that a null-basis basis-blade product expands to **at most 2 blades** (128 of 1024 pairs vanish, 768 single, 128 double) with **every coefficient exactly ±1** — so the product adds **no new rounding** and the rewrite is "`_cga_geo_blades` returns up to two terms" plus its **5 call sites**, not a new engine. The null table agrees with the shipped orthonormal one under change of basis, 0 mismatches over 1024 pairs. And the payoff is confirmed: **0 of 25,500 full-mantissa triples over 2^-40..2^40 are non-null**, against 24.8% today. ⚠ My first ad-hoc derivation gave the wrong term distribution (256/448/320); the committed one carries a round-trip cross-check the ad-hoc version lacked. Original row follows. | Represent a conformal point in the null basis (n0 = (em-ep)/2, ninf = ep+em) so `q` lives in ONE coefficient and `P.P` cancels the same computed `q` against itself, exactly 0. ⚠ Touches the 32-slot blade product tables, not `cga_point`. Acceptance test exists: the 2.20.0 assertions pin the current band AND both known failures, so the repair must announce itself. | 2.20.0 |
