...
| **2.21.0 — the CGA null basis** | Represent a conformal point in the null basis (n0 = (em-ep)/2, ninf = ep+em) so `q` lives in ONE coefficient and `P.P` cancels the same computed `q` against itself, exactly 0. ⚠ Touches the 32-slot blade product tables, not `cga_point`. Acceptance test exists: the 2.20.0 assertions pin the current band AND both known failures, so the repair must announce itself. | 2.20.0 |
