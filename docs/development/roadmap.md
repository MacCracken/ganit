...
Suite **3937** across five harnesses (hisab 474, foundation 413, modules 2041, edge_cases 233,
abuse 776), constant gate **158/158**, **72** benchmarks, **35** `[lib]` modules, toolchain
...
| **2.20.0 — the four repairs 2.19.0 measured** | The items 2.19.0's decision fan-out proved are **repairs, not decisions**, each already reproduced under adversarial verification. **(1)** `_sym_render_f64`: the divergence from `fmt_float_buf` is not 30 values but the **entire finite |val| >= 2^63 band** (7,702 of 30,432 probed inputs, 961 of 2046 normal binades), and the SHIPPED tree is already wrong there — `sym_to_latex(-1e19)` renders `{--9223372036854775808.000000}`, a double minus. Decide the correct rendering above 2^63 first; both current forms are defective. **(2)** `cga_point` nullity: **not** a small-coordinate effect — with full-mantissa coordinates `cga_norm_sq` is non-zero at **500/500 trials in every binade from 2^-20 to 2^+20**, and 458/500 even in [1,2). The banding in the original filing is an artifact of a dyadic sweep making x² exact, the same shape as the 8-vs-536 ganita estimate. **(3)** The 28 subnormal SVD block ratios: the standing claim that they are "the GRID, not a defect" is **refuted** — scaling the block into the normal range answers all 75 ratios, worst 2 ulp. ⚠ **But do not ship that as it stands**: it was validated only on upper-triangular 2x2 blocks, and on a general 2x2 it converts LOUD failures into gross SILENT wrong answers, which is the very thing it was adopted to avoid. Widen the fixture family first. **(4)** The two `triangulate_polygon` items: every figure reproduced index-for-index under an independent exact integer simplicity test, but the proposed change over-reaches in the suite and under-reaches on the live source contract. ⚠ Its citation has now drifted three times for the same two lines (`collision_core.cyr` 740/742 → 762/764 → **actually 870/872**). ⭐ **A fifth item, found in passing and not on any list — now CLOSED as a documented trap, no code change.** `ad_grad` clears every adjoint and then scans from `root` to index 0, both over the FULL tape, so the obvious shared-tape Jacobian driver is **O(m^2) by construction**. Measured at n = 8 on a quiet box, the shared driver QUADRUPLES per doubling of m (461 -> 1240 -> 4667 -> 18398 us at m = 256/512/1024/2048) while the per-residual `ad_tape_reset` form DOUBLES (215 -> 391 -> 775 -> 1474 us). ⚠ **The 6.8x in the original note is one point on a curve** — the ratio is 2x at m = 256 and 12x at m = 2048 and grows without bound, so it must never be quoted as a constant. ⚠ No repair shipped: halving the clear leaves the scan O(root), so the driver stays quadratic, and the full clear is load-bearing because `ad_grad_of` above `root` must read 0. Sub-linear needs a reachability sweep, which is an algorithm change. Documented at the call site and tracked by two paired benchmark rows (72 -> 74). ⚠ Zero in-tree callers and zero consumer call sites, so nothing is hitting it today. | 2.19.0 |
