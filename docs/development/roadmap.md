...
| **2.16.0 — the small-angle series** | `su2_log`, `so3_log`, `se3_exp`, `se3_log`: replace the θ²/θ³ coefficients with their Taylor forms near 0, so the guard can drop to the exact-zero test the rest of the tree now uses. ⚠ Blocked on a series, not on a threshold — see the 2.15.0 row. | 2.15.0 |
